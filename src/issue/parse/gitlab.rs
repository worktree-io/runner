...
mod tests {
    ...
    fn parse_gitlab_url_valid() {
        let r =
            parse_gitlab_url("https://gitlab.com/acme/api/-/issues/42").unwrap();
        assert_eq!(
            r,
            IssueRef::GitLab {
                owner: "acme".into(),
    ...
    fn parse_gitlab_url_invalid_path() {
        let err =
            parse_gitlab_url("https://gitlab.com/acme/api/issues/42").unwrap_err();
    ...
    fn parse_gitlab_url_invalid_number() {
        let err =
            parse_gitlab_url("https://gitlab.com/acme/api/-/issues/abc").unwrap_err();
